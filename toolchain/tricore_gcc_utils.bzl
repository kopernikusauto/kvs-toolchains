...
_gen_hex = rule(
    ...
    attrs = dict(
        ...
        # _cc_toolchain = attr.label(default = Label("@kvs_toolchains//:current_tricore_toolchain")),
...
def aurix_binary(name, **kw):
    cc_binary(
        ...
        #toolchains = ["@kvs_toolchains//:current_tricore_toolchain"],
        **kw
    )
    _gen_hex(
        name = name + ".hex",
    ...
    native.alias(name = name, actual = name + ".hex")
